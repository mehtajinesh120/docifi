...
import { fileToBase64, formatBytes, optimizeImage } from "../utils/fileHelpers";
...
export const QuestionInput: React.FC<QuestionInputProps> = ({
  ...
  const handleFileSelect = async (file: File) => {
    try {
      ...
      if (isImg) {
        // Optimize high-resolution mobile camera photos to prevent network drop
        const { base64 } = await optimizeImage(rawBase64, 1400, 1400, 0.85);
        onQuestionFileChange({
          name: file.name,
          size: file.size,
          base64,
